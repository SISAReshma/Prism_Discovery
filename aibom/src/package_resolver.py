...
def _process_language(
    ...
    result["used_libraries"].setdefault(language, [])
    result["unused_libraries"].setdefault(language, [])
...
def resolve_and_compare(
    ...
    result = {
        "resolved_packages": [],
        "used_libraries": {"python": [], "javascript": [], "go": [], "dotnet": [], "java": []},
        "unused_libraries": {"python": [], "javascript": [], "go": [], "dotnet": [], "java": []},
        "resolution_summary": {
    ...
    if "go" in languages:
        _process_language(
            manifest=manifest_packages.get("go", []),
            imports_data=semgrep_imports.get("go_imports", []),
            language="go",
            result=result,
            method_counts=method_counts,
            is_js=False  # Go uses direct module paths like JS
        )
    
    if "dotnet" in languages:
        _process_language(
            manifest=manifest_packages.get("dotnet", []),
            imports_data=semgrep_imports.get("dotnet_imports", []),
            language="dotnet",
            result=result,
            method_counts=method_counts,
            is_js=False  # .NET uses direct namespace matching
        )
    
    if "java" in languages:
        _process_language(
            manifest=manifest_packages.get("java", []),
            imports_data=semgrep_imports.get("java_imports", []),
            language="java",
            result=result,
            method_counts=method_counts,
            is_js=False  # Java uses direct package matching
        )
