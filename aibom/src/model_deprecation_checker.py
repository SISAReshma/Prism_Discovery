...
def check_models_deprecation(model_names: List[str]) -> Dict:
    ...
    Returns:
        Dict with results for each model, including:
        - deprecated_count: total deprecated (all statuses)
        - shutdown_count: subset that are already shutdown
        - severity_breakdown: {CRITICAL: n, HIGH: n, MEDIUM: n, LOW: n, INFO: n}
    """
    checker = get_deprecation_checker()
    results = []
    deprecated_count = 0
    ...
    for model_name in model_names:
        ...
        if result:
            deprecated_count += 1
            # Track shutdown vs active-deprecated
