...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self._limits = {
            ...
            'npm': 300,           # npm registry: fairly generous
    ...
    def set_limit(self, endpoint: str, limit: int, window: int = 3600):
        ...
        self._window = window
        logger.info(f"Rate limit set for {endpoint}: {limit} calls per {window}s")
    ...
    def wait_if_needed(self, endpoint: str, max_wait: int = 60) -> bool:
        ...
        usage = self.get_current_usage(endpoint)
        wait_time = min(usage['reset_in'], max_wait)
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.error(f"[ERROR] {endpoint}: Unexpected error: {e}")
                    raise
