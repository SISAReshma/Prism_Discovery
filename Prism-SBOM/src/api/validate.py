...
async def validate_github_repo(repo_url: str, repo_type: str, pat: Optional[str] = None) -> dict:
    ...
    Args:
        ...
        pat: Personal Access Token (required for private repos)
    ...
    if repo_type == "private" and not pat:
        raise_validation_error("PAT_REQUIRED", "Personal Access Token is required for private repositories")
    ...
    try:
        ...
        if resp.status_code == 404:
            if repo_type == "public":
                raise_api_error("REPO_NOT_FOUND", f"Repository '{owner}/{repo}' not found or is private")
            else:
        ...
        # Verify repo type matches
        if repo_type == "public" and is_private:
            raise_validation_error(
                "REPO_IS_PRIVATE",
                f"Repository '{owner}/{repo}' is private. Use /validate/repo_private with a PAT."
            )
