...
from sbom.src.config.config import NPM_API

# Enable local file cache for npm results
try:
    from sbom.src.utils.cache_manager import get_npm_cache, set_npm_cache
...
class NpmClient:
    ...
    def _make_request(self, url: str) -> Optional[requests.Response]:
        ...
        if not _rate_limiter.wait_if_needed("npm", max_wait=120):
            logger.warning("[RATE LIMIT] npm: Rate limit exceeded, skipping request")
            return None
        
        # Record the call; if it fails the window is still full, skip
        if not _rate_limiter.record_call("npm"):
            logger.warning("[RATE LIMIT] npm: Rate limit exceeded after wait, skipping request")
            return None
