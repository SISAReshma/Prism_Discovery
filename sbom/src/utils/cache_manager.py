...
def set_cache(cache_type: str, key: str, data: Any) -> bool:
    ...
    with _cache_lock:
        try:
            ...
            logger.warning(f"Failed to cache {cache_type}/{key}: {type(e).__name__}: {e}")
            return False
...
def _minimize_depsdev_metadata(raw_meta: Dict, package: str, version: str) -> Dict:
    ...
    # Release date - deps.dev returns publishedAt timestamp
    release_date = raw_meta.get("publishedAt") or raw_meta.get("release_date") or ""
...
def get_depsdev_depgraph_cache(ecosystem: str, package: str, version: str) -> Optional[Dict]:
    """Get cached deps.dev dependency graph for a package."""
    key = f"depgraph_{ecosystem}_{package}_{version}"
    return get_cache("depsdev", key, TTL_DEPSDEV)


def set_depsdev_depgraph_cache(ecosystem: str, package: str, version: str, depgraph: Dict) -> bool:
    """Cache deps.dev dependency graph for a package."""
    key = f"depgraph_{ecosystem}_{package}_{version}"
    return set_cache("depsdev", key, depgraph)
...
# =============================================================================
# ADDITIONAL REGISTRY CACHE HELPERS
# =============================================================================

TTL_DEFAULT = 168  # 7 days

def get_cargo_cache(name: str, version: str) -> Optional[Dict]:
    key = f"{name}_{version}"
    return get_cache("cargo", key, TTL_DEFAULT)

def set_cargo_cache(name: str, version: str, metadata: Dict) -> bool:
    key = f"{name}_{version}"
    return set_cache("cargo", key, metadata)

def get_maven_cache(group_id: str, artifact_id: str, version: str) -> Optional[Dict]:
    key = f"{group_id}_{artifact_id}_{version}"
    return get_cache("maven", key, TTL_DEFAULT)

def set_maven_cache(group_id: str, artifact_id: str, version: str, metadata: Dict) -> bool:
    key = f"{group_id}_{artifact_id}_{version}"
    return set_cache("maven", key, metadata)

def get_rubygems_cache(name: str, version: str) -> Optional[Dict]:
    key = f"{name}_{version}"
    return get_cache("rubygems", key, TTL_DEFAULT)

def set_rubygems_cache(name: str, version: str, metadata: Dict) -> bool:
    key = f"{name}_{version}"
    return set_cache("rubygems", key, metadata)

def get_packagist_cache(name: str, version: str) -> Optional[Dict]:
    key = f"{name}_{version}"
    return get_cache("packagist", key, TTL_DEFAULT)

def set_packagist_cache(name: str, version: str, metadata: Dict) -> bool:
    key = f"{name}_{version}"
    return set_cache("packagist", key, metadata)

def get_go_cache(module: str, version: str) -> Optional[Dict]:
    key = f"{module}_{version}".replace("/", "_")
    return get_cache("go", key, TTL_DEFAULT)

def set_go_cache(module: str, version: str, data: Dict) -> bool:
    key = f"{module}_{version}".replace("/", "_")
    return set_cache("go", key, data)
