...
def cleanup_workspace(workspace: Path, scan_id: str, temp_base: Path) -> bool:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            logger.warning("Cleanup attempt failed", extra={"attempt": attempt, "workspace": str(workspace), "error": str(e)})
    ...
    try:
        ...
        if remaining:
            logger.warning("Cleanup incomplete", extra={"workspace": str(workspace), "remaining_items": len(remaining)})
            return False
        else:
            logger.debug("Cleaned workspace", extra={"workspace": str(workspace)})
            return True
    except Exception:
        return False
