...
from sbom.src.utils.file_utils import ensure_dir, extract_zip
from sbom.src.utils.git_utils import git_clone  # git_clone should accept token/username optional
from sbom.src.registry.language_registry import iter_manifest_patterns, get_purl_type, get_cataloger_instances, get_language_for_manifest
...
def _discover_manifests(workspace: Path) -> List[str]:
    """
    Walk the workspace and return list of notable manifest file paths.
    Supports both exact names (requirements.txt) and glob patterns (*.csproj).
    
    Fixed: Uses fnmatch for glob patterns like *.csproj, *.fsproj, *.gemspec.
    """
    import fnmatch
    manifests = []
    all_patterns = list(iter_manifest_patterns())
    # Separate exact names from glob patterns for efficient matching
    exact_names = set()
    glob_patterns = []
    ...
    for root, dirs, files in os.walk(workspace):
        ...
        for f in files:
            # Check exact match first (O(1) set lookup)
            if f in exact_names:
                full = root_path / f
                manifests.append(str(full))
                print(f"   * Found manifest: {full}")
            else:
                # Check glob patterns (e.g., *.csproj, *.fsproj, *.gemspec)
...
def _run_catalogers(workspace: Path,
                    nvd_api_key: Optional[str] = None) -> Tuple[List[Dict[str, Any]], List[str], List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    """
    ...
    Returns:
        Tuple of (packages list, manifests list, raw cataloger outputs, lock_data dict)
    """
    packages: List[Dict[str, Any]] = []
    manifests: List[str] = []
    raw_outputs: List[Dict[str, Any]] = []  # Store raw cataloger outputs for audit
    all_lock_data: Dict[str, Dict[str, Any]] = {}  # OPTIMIZED: {pkg_name: {version, hashes, dependencies}}
    ...
    for cataloger in catalogers:
        try:
            ...
            if isinstance(catres, dict) and "packages" in catres:
                ...
                # Collect lock data (optimized format: dict lookup instead of list)
                lock_data = catres.get("lock_data", {})
                if lock_data:
                    all_lock_data.update(lock_data)
                    print(f"   [LOCK] Collected {len(lock_data)} packages from lock files (optimized)")
                ...
                raw_outputs.append({
                    ...
                    "has_lock_file": catres.get("has_lock_file", False)
                })
    ...
    if all_lock_data:
        print(f"[INFO] Lock data collected: {len(all_lock_data)} packages (optimized dict lookup)")
    
    return packages_out, manifests, raw_outputs, all_lock_data
...
def scan_workspace(workspace: Path,
    ...
    packages, catalog_manifests, raw_outputs, _ = _run_catalogers(
        workspace,
    ...
    try:
        ...
        if verify_mode != "none":
            ...
            from sbom.src.clients import depsdev_client
