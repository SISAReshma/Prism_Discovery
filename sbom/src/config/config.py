...
# Package Registry URLs (for display/links - override for private registries)
NUGET_BASE_URL = os.environ.get("STACKSQ_NUGET_URL", "https://www.nuget.org/packages")
RUBYGEMS_BASE_URL = os.environ.get("STACKSQ_RUBYGEMS_URL", "https://rubygems.org/gems")
CARGO_BASE_URL = os.environ.get("STACKSQ_CARGO_URL", "https://crates.io/crates")
GO_BASE_URL = os.environ.get("STACKSQ_GO_URL", "https://pkg.go.dev")
MAVEN_BASE_URL = os.environ.get("STACKSQ_MAVEN_URL", "https://mvnrepository.com/artifact")
PACKAGIST_BASE_URL = os.environ.get("STACKSQ_PACKAGIST_URL", "https://packagist.org/packages")
COCOAPODS_BASE_URL = os.environ.get("STACKSQ_COCOAPODS_URL", "https://cocoapods.org/pods")

# External API Endpoints (for fetching metadata)
...
NUGET_API = os.environ.get("STACKSQ_NUGET_API", "https://api.nuget.org/v3")
RUBYGEMS_API = os.environ.get("STACKSQ_RUBYGEMS_API", "https://rubygems.org/api/v1")
CRATES_IO_API = os.environ.get("STACKSQ_CARGO_API", "https://crates.io/api/v1")
GO_PROXY_API = os.environ.get("STACKSQ_GO_API", "https://proxy.golang.org")
GO_SUM_API = os.environ.get("STACKSQ_GO_SUM_API", "https://sum.golang.org")
MAVEN_CENTRAL_API = os.environ.get("STACKSQ_MAVEN_API", "https://search.maven.org/solrsearch/select")
MAVEN_CENTRAL_REPO = os.environ.get("STACKSQ_MAVEN_REPO", "https://repo1.maven.org/maven2")
PACKAGIST_API = os.environ.get("STACKSQ_PACKAGIST_API", "https://repo.packagist.org")
COCOAPODS_API = os.environ.get("STACKSQ_COCOAPODS_API", "https://trunk.cocoapods.org/api/v1")
ANACONDA_API = os.environ.get("STACKSQ_ANACONDA_API", "https://api.anaconda.org")
CONAN_API = os.environ.get("STACKSQ_CONAN_API", "https://center.conan.io/v2")
VCPKG_API = os.environ.get("STACKSQ_VCPKG_API", "https://vcpkg.io/output.json")
CONAN_BASE_URL = os.environ.get("STACKSQ_CONAN_URL", "https://conan.io/center")

# Vulnerability & Security APIs
...
EXCLUDED_DIRECTORIES = frozenset({
    ...
    ".vscode", ".idea", ".vs",
    # Testing/Cache
    ".tox", ".pytest_cache", "__pycache__", ".mypy_cache", ".cache", ".coverage",
    # Temporary
    "temp", "tmp", ".tmp",
})

# Additional directories to skip for MANIFEST detection only
EXCLUDED_MANIFEST_DIRS = frozenset({
    "example", "examples",
    "test", "tests", "testing",
    "demo", "demos",
    "sample", "samples",
...
# Combined set for catalogers (EXCLUDED_DIRECTORIES + EXCLUDED_MANIFEST_DIRS)
CATALOGER_EXCLUDED_DIRS = EXCLUDED_DIRECTORIES | EXCLUDED_MANIFEST_DIRS
...
def generate_cert_in_identifier(ecosystem: str, name: str, version: str, supplier: str = None) -> str:
    ...
    if supplier:
        # Handle non-string supplier (e.g., list of developers from Maven)
        if isinstance(supplier, (list, tuple)):
            supplier = supplier[0] if supplier else ""
            if isinstance(supplier, dict):
                supplier = supplier.get("name") or supplier.get("id") or str(supplier)
...
# =============================================================================
# UNUSED-DEPENDENCY CLASSIFICATION
# =============================================================================

# Map SBOM language strings to the keys expected by resolve_and_compare()
SBOM_LANG_TO_RESOLVER = {
    "python": "python", "pip": "python",
    "javascript": "javascript", "npm": "javascript", "node": "javascript",
    "typescript": "javascript", "ts": "javascript",
    "go": "go", "golang": "go",
    "dotnet": "dotnet", ".net": "dotnet", "csharp": "dotnet", "c#": "dotnet",
    "nuget": "dotnet", "fsharp": "dotnet",
    "java": "java", "maven": "java", "gradle": "java",
}

# Languages supported by the semgrep import scanner
SEMGREP_SUPPORTED_LANGS = {"python", "javascript", "go", "dotnet", "java"}

# Known dev / build / test tools — these are NOT imported in application code.
# Normalised to lowercase; comparison uses .lower().
KNOWN_DEV_TOOLS: set = {
    # ── Python ──
    "pytest", "pytest-cov", "pytest-xdist", "pytest-asyncio", "pytest-mock",
    "pytest-httpx", "pytest-django", "pytest-flask", "pytest-timeout",
    "unittest2", "nose", "nose2", "tox", "nox",
    "black", "autopep8", "yapf", "isort", "ruff",
    "flake8", "pylint", "pyflakes", "pycodestyle", "pydocstyle", "bandit",
    "mypy", "pyright", "pytype",
    "pyinstaller", "cx-freeze", "py2exe", "nuitka", "briefcase",
    "setuptools", "wheel", "build", "twine", "flit", "hatch", "hatchling",
    "poetry", "poetry-core", "pdm", "pipenv",
    "sphinx", "sphinx-rtd-theme", "mkdocs", "mkdocs-material", "pdoc", "pydoc",
    "coverage", "codecov", "coveralls",
    "pre-commit", "commitizen", "bump2version", "bumpversion",
    "ipython", "ipdb", "pdb", "debugpy",
    "notebook", "jupyterlab", "jupyter",
    "faker", "factory-boy", "hypothesis", "responses", "vcrpy",
    "moto", "localstack",
    # ── JavaScript / TypeScript ──
    "eslint", "prettier", "stylelint", "lint-staged", "husky",
    "jest", "mocha", "chai", "sinon", "nyc", "c8", "vitest", "cypress",
    "playwright", "puppeteer",
    "webpack", "webpack-cli", "webpack-dev-server",
    "rollup", "esbuild", "parcel", "vite", "turbopack",
    ...
    # ── Java ──
    "junit", "junit-jupiter", "mockito", "jacoco", "checkstyle", "spotbugs",
    "pmd", "findbugs", "errorprone",
    # ── .NET ──
    "nunit", "xunit", "mstest", "moq", "coverlet",
    "stylecop", "fxcop",
}

# Runtime-optional packages — invoked via CLI / plugin mechanism, not imported
RUNTIME_OPTIONAL: set = {
    "gunicorn", "uvicorn", "waitress", "daphne", "hypercorn",
    "celery", "dramatiq", "rq", "huey",
    "alembic", "flask-migrate",
    "pip", "pip-tools", "pipdeptree",
    "docker-compose", "ansible",
    "pm2", "forever",
}


def classify_unused_package(pkg_name: str, scope: str | None = None) -> str:
    """Classify an unused package into one of three categories.

    Returns:
        "dev_tool"          – known dev / build / test tool, or cataloger-tagged scope=dev
        "runtime_optional"  – invoked via CLI / plugin, not imported
        "truly_unused"      – not in any known list → candidate for removal
    """
    normalised = pkg_name.lower().replace("_", "-")
    if scope and scope.lower() == "dev":
        return "dev_tool"
    if normalised in KNOWN_DEV_TOOLS:
        return "dev_tool"
    if normalised in RUNTIME_OPTIONAL:
