...
class NpmCataloger(BaseCataloger):
    ...
    def catalog(self, repo_root: str, token: Optional[str] = None, nvd_api_key: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Collect direct dependency names from package.json for direct/transitive classification
        direct_names: set = set()
        for m in manifests:
            if m.name == "package.json":
                try:
                    pj = json.loads(m.read_text(encoding="utf-8"))
                    for dep_name in (pj.get("dependencies", {}) or {}).keys():
                        direct_names.add(dep_name.lower())
                    for dep_name in (pj.get("devDependencies", {}) or {}).keys():
                        direct_names.add(dep_name.lower())
                except Exception:
                    pass

        # Build lock_data from lock file entries: {pkg_name: {version, hashes, dependencies}}
        lock_data: Dict[str, Dict[str, Any]] = {}
        has_lock_file = False
        for m in manifests:
            if m.name in ("package-lock.json", "yarn.lock", "pnpm-lock.yaml"):
                has_lock_file = True
                break

        # dedupe — prefer lock file entries (they have exact versions + hashes)
        ...
        for p in packages:
            ...
            if existing is None:
                seen[key] = p
            elif p.get("hashes") and not existing.get("hashes"):
                # Lock file entry has hashes, manifest entry doesn't — prefer lock
                seen[key] = p
            elif p.get("version") != "UNKNOWN" and existing.get("version") in ("UNKNOWN", ""):
                seen[key] = p
        packages = list(seen.values())

        # Collect lock_data from lock file parsed packages
        for p in packages:
            name = p.get("name", "")
            version = p.get("version", "")
            if name and version and version != "UNKNOWN" and p.get("hashes"):
                key = name.lower()
        ...
        if lock_data:
            print(f"[INFO] Collected {len(lock_data)} packages from lock files for transitive resolution")

        print(f"[INFO] Found {len(packages)} dependencies from manifests ({len(direct_names)} direct)")
        ...
        for pkg in packages:
            ...
            # Mark direct vs transitive based on package.json declarations
            pkg["is_direct_dependency"] = name.lower() in direct_names if direct_names else True
            
            # Version resolution fields
            name_lower = name.lower()
            if name_lower in lock_data:
        ...
        # Log version resolution summary
        ...
        try:
            ...
            if "packages" in j and lockfile_version >= 2:
                ...
                for pkg_path, pkg_info in packages_data.items():
                    ...
                    # Extract dependencies as structured dicts
                    raw_deps = pkg_info.get("dependencies", {})
                    deps = [
                        {
                            "name": d,
                            "version_constraint": str(v),
                            "purl": f"pkg:npm/{d}@{v}" if v else f"pkg:npm/{d}",
                        }
            ...
            # v1 format uses "dependencies" key with nested structure
                ...
                def flatten_deps(deps_dict, parent_path=""):
                    for name, info in deps_dict.items():
                        ...
                        raw_requires = info.get("requires", {})
                        requires = [
                            {
                                "name": d,
                                "version_constraint": str(v),
            ...
            # Dependency entries (4 spaces indentation)
                ...
                if '"' in line_stripped:
                    ...
                    if len(parts) >= 4:
                        ...
                        dep_constraint = parts[3] if len(parts) > 3 else ""
                        if dep_name:
                            current_deps.append({
                                "name": dep_name,
                                "version_constraint": dep_constraint,
                                "purl": f"pkg:npm/{dep_name}",
                            })
                    elif len(parts) >= 2:
                        dep_name = parts[1] if parts[0] == '' else parts[0]
                        if dep_name:
        ...
        for pkg_key, pkg_info in packages_data.items():
            ...
            if isinstance(pkg_info.get("dependencies"), dict):
                deps = [
                    ...
                    for d, v in pkg_info["dependencies"].items()
                ]
