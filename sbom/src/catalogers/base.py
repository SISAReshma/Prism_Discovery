...
class BaseCataloger(ABC):
    ...
    # -------------------------
    def _should_skip_path(self, path: Path, root: Path = None) -> bool:
        """
        Skip manifest files in excluded directories (example, test, demo, etc.).
        
        Uses relative path from workspace root to avoid false positives
        from parent directories (e.g., temp/ in the workspace path).
        
        Args:
            path: Full path to manifest file
            root: Workspace root path (used to compute relative path)
        """
        from sbom.src.config.config import CATALOGER_EXCLUDED_DIRS
        ...
        parts_lower = [p.lower() for p in check_path.parent.parts]
        return any(excluded in parts_lower for excluded in CATALOGER_EXCLUDED_DIRS)
